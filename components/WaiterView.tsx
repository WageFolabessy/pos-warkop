...
export const WaiterView: React.FC<WaiterViewProps> = ({
  ...
  const [editingNotesItem, setEditingNotesItem] = useState<OrderItem | null>(
    null,
  );
  ...
  const occupiedTables = useMemo(() => {
    return tables.filter(
      (t) => t.status === "belum_lunas" && t.items.length > 0,
    );
  }, [tables]);
          ...
          <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    {takeaway?.kitchenStatus === "dimasak" && (
                      <span className="text-[10px] text-gray-600 font-medium bg-gray-50 border border-gray-200 px-1.5 py-0.5 rounded">
                        {(() => {
                          const done = takeaway.items.filter((it) =>
                            (takeaway.completedItemIds || []).includes(
                              it.menuItem.id,
                            ),
                          ).length;
                          return done > 0
                            ? `${done}/${takeaway.items.length} Siap`
                            : "Diracik";
                        })()}
                      </span>
                    )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2.5">
              {physicalTables.map((tbl) => {
                const totalItems = tbl.items.reduce(
                  (sum, it) => sum + it.quantity,
                  0,
                );
                const isOccupied = tbl.status === "belum_lunas";
                const isSelected = activeTargetId === tbl.targetId;
                const doneCount = tbl.items.filter((it) =>
                  (tbl.completedItemIds || []).includes(it.menuItem.id),
                ).length;
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {tbl.kitchenStatus === "dimasak" && (
                          <span className="text-[9px] text-gray-600 font-medium bg-gray-50 border border-gray-200 px-1.5 py-0.5 rounded">
                            {doneCount > 0
                              ? `${doneCount}/${tbl.items.length} Siap`
                              : "Diracik"}
                          </span>
                        )}
                        <span
          ...
          <div className="flex-1 overflow-y-auto p-3 sm:p-4 pb-28">
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-2.5 sm:gap-3">
              {filteredItems.map((item) => {
                ...
                const existingOrderItem = draftItems.find(
          ...
          <div className="relative z-10 w-full max-h-[85vh] bg-white rounded-t-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom duration-150">
            ...
            <div className="flex-1 overflow-y-auto p-4 divide-y divide-gray-100 space-y-3">
              {draftItems.length === 0 ? (
                ...
                draftItems.map((it) => {
                  const isItemDone = (
                    activeTable?.completedItemIds || []
                  ).includes(it.menuItem.id);
                  return (
                      ...
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <div className="flex items-center border border-gray-200 rounded-full bg-gray-50 overflow-hidden">
                            <button
                              onClick={() =>
                                onUpdateQuantity(it.menuItem.id, -1)
                              }
              ...
              {occupiedTables.length === 0 ? (
                <div className="py-16 text-center text-gray-400 text-xs">
                  ...
                  <p className="text-gray-400 mt-0.5">
                    Semua meja berstatus kosong / pesanan sudah diselesaikan di
                    kasir.
                  </p>
                </div>
              ) : (
                occupiedTables.map((tbl) => {
                  const tblItemTotal = tbl.items.reduce(
                    (s, it) => s + it.quantity,
                    0,
                  );
                  const doneCount = tbl.items.filter((it) =>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="space-y-1.5 pt-0.5">
                        {tbl.items.map((it) => {
                          const isDone = (tbl.completedItemIds || []).includes(
                            it.menuItem.id,
                          );
                          return (
                            <div
